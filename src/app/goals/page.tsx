    ...
    return (
      <div style={{ padding: "0 20px" }}>
        <div style={{ padding: "20px 0 12px", display: "flex", alignItems: "center", gap: 12 }}>
          ...
          <h1 className="font-display" style={{ fontSize: 24, fontWeight: 600, margin: 0 }}>New Goal</h1>
        </div>
        ...
        <div className="stagger-children" style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          ...
          <div className="card" style={{ padding: 16 }}>
            ...
            <input type="text" value={title} onChange={(e) => setTitle(e.target.value)} placeholder="e.g. Run a 5K" autoFocus maxLength={100}
              style={{ width: "100%", padding: "10px 12px", fontSize: 15, borderRadius: "var(--radius-sm)", border: "1.5px solid var(--surface-border)", background: "var(--bg)", color: "var(--text)", outline: "none", marginTop: 6, fontFamily: "var(--font-sans)" }} />
          </div>
          ...
          <div className="card" style={{ padding: 16 }}>
            ...
            <textarea value={description} onChange={(e) => setDescription(e.target.value)} placeholder="Why does this matter to you?" maxLength={500} rows={3}
              style={{ width: "100%", padding: "10px 12px", fontSize: 15, borderRadius: "var(--radius-sm)", border: "1.5px solid var(--surface-border)", background: "var(--bg)", color: "var(--text)", outline: "none", marginTop: 6, fontFamily: "var(--font-sans)", resize: "vertical" }} />
          </div>
          ...
          <div className="card" style={{ padding: 16 }}>
            ...
            {tasks.map((t) => (
              <div key={t.id} style={{ display: "flex", alignItems: "center", gap: 8, padding: "6px 0", borderBottom: "1px solid var(--surface-border)" }}>
                <span style={{ flex: 1, fontSize: 14, color: "var(--text)" }}>{t.title}</span>
                <button onClick={() => removeTask(t.id)} aria-label="Remove"
                  style={{ background: "none", border: "none", color: "var(--danger)", cursor: "pointer", fontSize: 18, padding: "4px 8px", minWidth: 32, minHeight: 32 }}>
                  ×
                </button>
              </div>
            ))}
            <div style={{ display: "flex", gap: 8, marginTop: 8 }}>
              <input type="text" value={newTask} onChange={(e) => setNewTask(e.target.value)} placeholder="Add a step..."
                ...
                style={{ flex: 1, padding: "10px 12px", fontSize: 14, borderRadius: "var(--radius-sm)", border: "1.5px solid var(--surface-border)", background: "var(--bg)", color: "var(--text)", outline: "none", fontFamily: "var(--font-sans)" }} />
    ...
    return (
      <div style={{ padding: "0 20px" }}>
        <div style={{ padding: "20px 0 12px", display: "flex", alignItems: "center", gap: 12 }}>
          ...
          <div style={{ flex: 1 }}>
            <h1 className="font-display" style={{ fontSize: 22, fontWeight: 600, margin: 0 }}>{selectedGoal.title}</h1>
          </div>
        ...
        <div className="stagger-children" style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          ...
          <div className="card" style={{ padding: 16 }}>
            <div style={{ display: "flex", gap: 12, alignItems: "center", marginBottom: 12 }}>
              <span style={{ fontSize: 14, background: "var(--primary-glow)", padding: "4px 12px", borderRadius: "var(--radius-xl)", color: "var(--primary)", fontWeight: 500 }}>
                {getFocusEmoji(selectedGoal.focusArea)} {getFocusLabel(selectedGoal.focusArea)}
              </span>
              {selectedGoal.completed && (
                <span style={{ fontSize: 13, background: "rgba(64,145,108,0.1)", padding: "4px 12px", borderRadius: "var(--radius-xl)", color: "var(--success)", fontWeight: 600 }}>
                  Completed
                </span>
              )}
            </div>
          ...
          <div className="card" style={{ padding: 16 }}>
            <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 8 }}>
              ...
              <span style={{ fontSize: 14, fontWeight: 600, color: "var(--primary)" }}>{progress}%</span>
            </div>
            <div style={{ height: 8, borderRadius: 4, background: "var(--bg-secondary)", overflow: "hidden" }}>
              <div style={{ height: "100%", width: `${progress}%`, borderRadius: 4, background: "var(--primary)", transition: "width 0.3s ease" }} />
            </div>
          </div>
              ...
              {selectedGoal.tasks.map((task) => (
                <button key={task.id} onClick={() => toggleTask(selectedGoal.id, task.id)}
                  style={{
                    ...
                    borderBottom: "1px solid var(--surface-border)", width: "100%",
                    background: "none", border: "none", borderBottomStyle: "solid", borderBottomWidth: 1, borderBottomColor: "var(--surface-border)",
                  ...
                  <span style={{ fontSize: 15, color: task.completed ? "var(--text-muted)" : "var(--text)", textDecoration: task.completed ? "line-through" : "none" }}>
                    {task.title}
                  </span>
                </button>
              ))}
          ...
          <button onClick={() => toggleGoalComplete(selectedGoal.id)}
            ...
            {selectedGoal.completed ? "Mark as Incomplete" : "Mark Goal Complete"}
          </button>
        </div>
      </div>
    );
            ...
            return (
              <button key={goal.id} onClick={() => { setSelectedId(goal.id); setView("detail"); }}
                ...
                <div style={{ height: 6, borderRadius: 3, background: "var(--bg-secondary)", overflow: "hidden" }}>
                  <div style={{ height: "100%", width: `${progress}%`, borderRadius: 3, background: "var(--primary)", transition: "width 0.3s ease" }} />
                </div>
                {goal.tasks.length > 0 && (
